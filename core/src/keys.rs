...
/// directly. A failing system generator is not recoverable.
#[must_use]
...
#[inline]
fn derive_domain_key(domain: &[u8], input: &[u8]) -> [u8; 32] {
...
#[must_use]
#[inline]
pub fn domain_dht_key(domain: &[u8], input: &[u8]) -> DhtKeyId {
    DhtKeyId::from_bytes(derive_domain_key(domain, input))
}
...
pub fn oidc_subject_key(issuer: &str, subject_id: &str) -> [u8; 32] {
    ...
    derive_domain_key(b"oidc-subject-v1", &input)
}
...
pub fn realm_presence_key(realm_id: &RealmId) -> DhtKeyId {
    domain_dht_key(b"realm-presence", realm_id.as_bytes())
}
...
pub fn realm_endpoint_key(realm_id: &RealmId, node_id: &NodeId) -> DhtKeyId {
    ...
    domain_dht_key(b"realm-endpoint-v1", &input)
}

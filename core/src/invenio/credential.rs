...
impl InvenioCredential {
    pub fn seal(
        ...
        let sealed =
            EncryptedS3Secret::encrypt(key, token, &token_aad(user, group, connector, &endpoint))
    ...
    pub fn open(
        ...
        self.sealed
            .open(key, &token_aad(user, group, connector, endpoint))
            .map_err(|_| InvenioError("repository login is not valid for this user and node"))
    }
}

fn token_aad(user: UserId, group: Ulid, connector: Ulid, endpoint: &str) -> Vec<u8> {
    let mut aad = b"aruna invenio credential\0".to_vec();

//! Tests that group and realm role and assignment events materialize and detect conflicts.
...
#[test]
fn group_role_exists() {
    let mut state = group_state();
    let role_id = role_id(3);
    let user_id = user_id_seed(4);

    state
        .apply(&assign_group_user(1, 1, role_id, user_id))
        .unwrap();

    assert!(state.materialized_group_roles().is_empty());
    assert!(state.materialized_group_assignments().is_empty());
...
#[test]
fn concurrent_group_order() {
    let role_id = role_id(3);
    let first_user_id = user_id_seed(4);
    let second_user_id = user_id_seed(5);
    let role_seed = add_group_role(1, 1, role_id);
    let first_assignment = assign_group_user(2, 2, role_id, first_user_id);
    let second_assignment = assign_group_user(3, 3, role_id, second_user_id);
    ...
    assert_eq!(left.materialized_group_assignments(), expected);
    assert_eq!(right.materialized_group_assignments(), expected);
    assert_eq!(left.conflicts, right.conflicts);
    assert!(left.conflicts.is_empty());
}

#[test]
fn concurrent_role_order() {
...
fn concurrent_role_closed() {
    ...
    state
        ...
        .unwrap();
    state
        .apply(&remove_group_assignment(3, 3, role_id, user_id))
        .unwrap();
...
fn realm_role_materialize() {
    ...
    state
        .apply(&assign_realm_user(2, 2, role_id, user_id))
        .unwrap();

    assert_eq!(state.materialized_realm_roles(), BTreeSet::from([role_id]));
    assert_eq!(
        state.materialized_realm_assignments(),
        BTreeMap::from([(role_id, BTreeSet::from([user_id]))])
    );
    assert!(state.conflicts.is_empty());
}
...
fn concurrent_realm_closed() {
    ...
    state
        ...
        .unwrap();
    state
        .apply(&remove_realm_assignment(3, 3, role_id, user_id))
        .unwrap();

    assert_eq!(state.materialized_realm_roles(), BTreeSet::from([role_id]));
    assert!(state.materialized_realm_assignments().is_empty());
    let conflict = state
        .conflicts

...
pub(in crate::document_sync) async fn store_registry_upsert(
    ...
    let mut base_entries =
        registry_write_entries(&record).map_err(|error| NetError::Bootstrap(error.to_string()))?;
    ...
    for _ in 0..2 {
        ...
        match record_fenced_txn(storage, &record, txn_id).await {
            Ok(true) => {
                ...
                let stale = match transaction_read(
                    storage,
                ...
                let deletes = registry_delete_entries(stale.as_ref().unwrap_or(&record));
                match replace_batch_in(storage, txn_id, deletes, Vec::new()).await {
        ...
        match transaction_placement_fence(storage, &record, txn_id).await {
        ...
        let existing_value = match transaction_read(
            storage,
            target.storage_keyspace().to_string(),
            target.storage_key(),
            Some(txn_id),
        ...
        if let Some(existing) = existing
            .as_ref()
            .filter(|existing| delete_present || registry_is_stale(existing, &record))
        {
            ...
            match replace_batch_in(storage, txn_id, Vec::new(), repairs).await {
                Ok(()) => return Ok(MetadataPlacementOutcome::Accepted(())),
                Err(NetError::Storage(StorageError::TransactionConflict)) => {
                    let _ = storage
                        .send_storage_effect(StorageEffect::AbortTransaction { txn_id })
        ...
        match replace_batch_in(storage, txn_id, Vec::new(), entries).await {
            Ok(()) => return Ok(MetadataPlacementOutcome::Accepted(())),
            Err(NetError::Storage(StorageError::TransactionConflict)) => {
                let _ = storage
                    .send_storage_effect(StorageEffect::AbortTransaction { txn_id })
...
pub(in crate::document_sync) async fn store_graph_lifecycle(
    ...
    let (key_space, key, _) =
        graph_lifecycle_entry(record).map_err(|error| NetError::Bootstrap(error.to_string()))?;
    for _ in 0..2 {
        let txn_id = start_storage_transaction(storage).await?;
        ...
        if !delete_matches_graph(&delete, record) || registry_live {
            let _ = storage
                .send_storage_effect(StorageEffect::AbortTransaction { txn_id })
                .await;
            return Ok(false);
...
pub(in crate::document_sync) async fn store_document_lifecycle(
    ...
    for _ in 0..2 {
        let txn_id = start_storage_transaction(storage).await?;
        let writes = match current_lifecycle_entries(storage, record, &change, txn_id).await {
            Ok(writes) => writes,
            Err(error) => {
                let _ = storage
                    .send_storage_effect(StorageEffect::AbortTransaction { txn_id })
        ...
        match replace_batch_in(storage, txn_id, deletes, writes.unwrap_or_default()).await {
            Ok(()) => return Ok(true),
            Err(NetError::Storage(StorageError::TransactionConflict)) => {
                let _ = storage
                    .send_storage_effect(StorageEffect::AbortTransaction { txn_id })
...
/// Folds a replicated PID mapping, its sidecar and manifest entry into one
/// monotonic transaction that never lets an Active row overwrite a Withdrawn
/// tombstone and fences the stamped placement against the document's shard.
...
pub(in crate::document_sync) async fn current_lifecycle_entries(
    ...
    if let Some(manifest) = shard_manifest_entry(&target, change)
        .map_err(|error| NetError::Bootstrap(error.to_string()))?
    {
...
pub(in crate::document_sync) fn registry_is_stale(
...
pub(in crate::document_sync) async fn registry_sidecar_repairs(
    ...
    for (key_space, key, value) in entries.into_iter().skip(1) {
        let current =
            transaction_read(storage, key_space.clone(), key.clone(), Some(txn_id)).await?;
...
pub(in crate::document_sync) async fn registry_cleanup_txn(
    ...
    let Some(value) = transaction_read(
        storage,
        target.storage_keyspace().to_string(),
        target.storage_key(),
        Some(txn_id),
    ...
    Ok(registry_delete_entries(&record))
}

pub(in crate::document_sync) async fn transaction_placement_fence(
    storage: &StorageHandle,
    record: &MetadataRegistryRecord,
    txn_id: TxnId,
...
/// The placement a structured metadata id must ride, derived inside the caller's
/// transaction and compared with the publisher's stamp. The transactional config
/// read is the fence; `group_id` is compared only when known.
...
pub(in crate::document_sync) fn document_delete_entries(
    ...
    let mut entries = vec![
        document_lifecycle_entry(&lifecycle)
            .map_err(|error| NetError::Bootstrap(error.to_string()))?,
        graph_lifecycle_entry(&record.tombstone)
            .map_err(|error| NetError::Bootstrap(error.to_string()))?,
    ];

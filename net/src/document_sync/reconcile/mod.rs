...
impl DocumentSyncService {
    /// Reconciles shard topic membership to the current holder set: the publisher
    /// cutover installs before controls. `history_cutoff` freezes only for
    /// `verified_topics`; retained draining peers stay members until flushed.
    pub async fn reconcile_shard_membership(
        ...
        verified_topics: &BTreeSet<::irokle::TopicId>,
    ) -> Result<()> {
        if topics.is_empty() {
            return Ok(());
        }
        ...
        let member_peers: BTreeSet<PeerId> = members
            ...
            .chain(retained.iter().map(node_to_peer))
            .collect();
        ...
        let topics: Vec<::irokle::TopicId> = topics
            .iter()
            .copied()
        ...
        for topic_id in topics {
            ...
            let current = publisher_peers
                .iter()
                .copied()
                .map(|peer| ::irokle::actor_id_for(topic_id, peer))
                .collect();
            match state {
                Some(state) => {
        ...
        for (topic_id, state) in states {
            ...
            let actor_id = ::irokle::actor_id_for(topic_id, local_peer);
...
impl DocumentSyncService {
    ...
    pub(in crate::document_sync) fn document_topic_ids(&self) -> Result<Vec<::irokle::TopicId>> {
        let topics = self
            .node
            .list_topics()
            .map_err(|error| NetError::Bootstrap(error.to_string()))?;
...
pub(in crate::document_sync) async fn dependency_available(
    storage: &StorageHandle,
    dependency: DocumentSyncDependency,
) -> Result<bool> {
    match dependency {
        DocumentSyncDependency::RealmConfig(realm_id) => {
            Ok(read_realm_config(storage, realm_id).await?.is_some())
        }
        DocumentSyncDependency::RealmAuthorization(realm_id) => {
            Ok(read_realm_authorization(storage, realm_id).await?.is_some())
        }
        DocumentSyncDependency::PlacementStrategy {
            realm_id,
            strategy_id,
        } => Ok(read_realm_config(storage, realm_id)
            .await?
            .is_some_and(|config| {

...
pub(super) async fn write_registry_record(
    ...
    let event = storage
        .send_storage_effect(StorageEffect::BatchWrite {
            writes: registry_write_entries(record).expect("registry entries build"),
            txn_id: None,
        })
        .await;
    assert!(matches!(
...
pub(super) async fn write_document_lifecycle(
    ...
    batch_write_to(
        storage,
        vec![document_lifecycle_entry(lifecycle).expect("document lifecycle entry builds")],
    )
    .await

//! Tests that role assignment events materialize group and realm roles and subject claims.
...
#[tokio::test]
async fn subject_claims_promote() {
...
#[tokio::test]
async fn seeded_group_materializes() {
    ...
    let assignment_path = group_user_path(&role_id, &assigned_user_id);
    assert_eq!(
        reducer_state
            .user_subject_ids
            .get(&assignment_path)
...
#[tokio::test]
async fn new_role_materializes() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn assignment_conflict_materializes() {
    let (_dir, storage) = test_storage();
    ...
    let target = AdminDocumentTarget::Group { group_id };
    let assignment_path = group_user_path(&role_id, &assigned_user_id);
    let conflict_key = reducer_conflict_key(&target, &assignment_path);
...
#[tokio::test]
async fn conflicting_add_removes() {
    let (_dir, storage) = test_storage();
    ...
    let assignment_path = realm_user_path(&role_id, &assigned_user_id);
    apply_admin_operation(
        &storage,
        document_target,
        AdminDocumentEvent {
    ...
    assert!(
        !read_realm_auth(&storage, realm_id).await.roles[&role_id]
            .assigned_users
            .contains(&assigned_user_id)
    );
}
...
#[tokio::test]
async fn seeded_realm_materializes() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn new_realm_materializes() {
    let (_dir, storage) = test_storage();
    ...
    let auth_doc = read_realm_auth(&storage, realm_id).await;
    let auth_role = &auth_doc.roles[&role_id];

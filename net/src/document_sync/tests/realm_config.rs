//! Tests realm config replication: materialization, relay origin checks, revocation and quotas.
...
#[tokio::test]
async fn settings_materialize_config() {
    let (_dir, storage) = test_storage();
    ...
    assert_eq!(
        reducer_state.materialized_realm_discovery(),
        Some(discovery)
    );
}

#[tokio::test]
async fn description_materializes_config() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn placement_materializes_config() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn dangling_strategy_materializes() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn settings_create_config() {
    let (_dir, storage) = test_storage();
...
async fn accepts_onboarded_origin() {
    ...
    let publisher = ::irokle::actor_id_for(topic, node_to_peer(&attacker.node_id));
...
async fn validate_relayed(fixture: &RelayFixture, publisher: NodeId) -> AdminEventValidation {
    validate_admin_event(
        &fixture.storage,
        fixture.topic,
        ::irokle::actor_id_for(fixture.topic, node_to_peer(&publisher)),
        &fixture.target,
        &fixture.event,
        fixture.realm_id,
        &fixture.placement,
...
async fn rejects_forged_relay() {
    ...
    assert!(matches!(
        validate_admin_event(
            &fixture.storage,
            fixture.topic,
            ::irokle::actor_id_for(fixture.topic, node_to_peer(&fixture.relay)),
            &fixture.target,
            &forged,
            fixture.realm_id,
            &fixture.placement,
...
async fn caps_flooding_origin() {
    ...
    batch_write_to(
        &storage,
        vec![reducer_state_entry(&state).expect("state serializes")],
    )
    .await
    ...
    assert!(matches!(
        validate_admin_event(
            &storage,
            topic,
            ::irokle::actor_id_for(topic, node_to_peer(&flooder.node_id)),
            &config_target,
            &flood_event,
            realm_id,
            &PlacementRef::NIL,
    ...
    assert_eq!(
        validate_admin_event(
            &storage,
            topic,
            ::irokle::actor_id_for(topic, node_to_peer(&neighbour.node_id)),
            &config_target,
            &neighbour_event,
            realm_id,
            &PlacementRef::NIL,
...
fn accepts_historical_origin() {
    ...
    assert!(
        state
            .conflicts
            .contains_key(&config_node_path(&origin.node_id))
    );
...
async fn replicated_revocation_compacts() {
    ...
    let mut state = read_reducer_state(&storage, &target)
        .await
...
async fn redundant_persists_clock() {
    ...
    apply_admin_operation(&storage, document_target.clone(), first)
        .await
    ...
    apply_admin_operation(&storage, document_target.clone(), second)
        .await
...
async fn join_authority_replication() {
    ...
    let auth = GroupAuthorizationDocument::default_group_doc(owner.user_id, realm_id, group_id);
    let role_id = auth
        .roles
        .values()
    ...
    let requested = reducer
        ...
        .unwrap();
    assert!(matches!(
        validate_group_authority(&storage, &requested, None)
            .await
            .unwrap(),
        AdminEventValidation::Accepted
    ));
    apply_admin_operation(&storage, document.clone(), requested)
        .await
        .unwrap();
    ...
    assert!(matches!(
        validate_group_authority(&storage, &denied, Some(&reducer))
            .await
            .unwrap(),
    ...
    let approved = reducer
        ...
        .unwrap();
    let previous = read_reducer_state(&storage, &reducer.target)
        .await
        .unwrap()
        .unwrap();
    assert!(matches!(
        validate_group_authority(&storage, &approved, Some(&previous))
            .await
            .unwrap(),
        AdminEventValidation::Accepted
    ));
    apply_admin_operation(&storage, document, approved)
        .await
        .unwrap();
    assert!(
    ...
    let saved = read_reducer_state(&storage, &reducer.target)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(
...
async fn policies_authority_gate() {
    ...
    // arm: a non-owner config admin is accepted, one without write rejected.
    let (_dir, storage) = test_storage();
    let realm_id = RealmId::from_bytes([59; 32]);
    ...
    for (permission, expect_accept) in [(Permission::WRITE, true), (Permission::READ, false)] {
        ...
        let validation = validate_group_authority(&storage, &event, None)
            .await
...
async fn quota_survives_materialization() {
    ...
    // reducer and later carried through materialized_realm_config.
    apply_admin_operation(
        &storage,
        document_target.clone(),
        test_admin_event(
...
#[tokio::test]
async fn settings_bootstrap_config() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn settings_conflict_withholds() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn oidc_conflict_withholds() {
    let (_dir, storage) = test_storage();
    ...
    let path = aruna_core::reducer::config_oidc_path("default");
    assert!(
        read_storage_value(
            &storage,
            DOCUMENT_CONFLICT_KEYSPACE,
...
#[tokio::test]
async fn kind_conflict_withholds() {
    let (_dir, storage) = test_storage();

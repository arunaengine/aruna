//! Tests metadata registry lifecycle: upserts, fences, tombstones and delete pruning.
...
#[tokio::test]
async fn stale_registry_skipped() {
    ...
    store_registry_upsert(&storage, stale, stale_bytes)
        .await
...
async fn registry_replay_repairs() {
    ...
    let mut entries = registry_write_entries(&record).expect("registry entries build");
    let primary = entries.remove(0);
    batch_write_to(&storage, vec![primary])
        .await
        .expect("registry primary writes");

    store_registry_upsert(
        &storage,
        record.clone(),
...
async fn registry_fence_ulid() {
    ...
    write_document_lifecycle(&storage, &delete).await;
...
async fn upsert_keeps_config() {
    // The placement fence reads the realm config inside its transaction and must
    // not write it back; an identical write conflicts the config's readers.
    let (_dir, storage) = test_storage();
    let realm_id = RealmId::from_bytes([42; 32]);
...
async fn capacity_retains_cursors() {
    ...
    assert!(
        !dependency_available(&storage, dependency)
            .await
...
#[tokio::test]
async fn stale_delete_preserves() {
    let (_dir, storage) = test_storage();
    ...
    write_document_lifecycle(&storage, &stale_delete).await;

    delete_registry_record(&storage, group_id, document_id)
        .await
        .expect("stale registry delete is fenced");

    assert_registry_present(&storage, &live).await;
}

#[tokio::test]
async fn tombstone_blocks_upsert() {
    let (_dir, storage) = test_storage();
    ...
    let outcome = store_registry_upsert(
        &storage,
        stale.clone(),
    ...
    assert_registry_deleted(&storage, group_id, document_id).await;
    assert_eq!(
        read_document_lifecycle(&storage, document_id).await,
        delete_lifecycle
    );
}

#[tokio::test]
async fn tombstone_required() {
    let (_dir, storage) = test_storage();
    ...
    assert_registry_present(&storage, &record).await;
    assert!(
        read_graph_lifecycle(&storage, &graph.graph_iri)
            .await
            .is_none()
    );
}
...
#[tokio::test]
async fn live_lifecycle_preserved() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn newer_registry_preserved() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn matching_tombstone_deletes() {
    let (_dir, storage) = test_storage();
    ...
    assert_eq!(
        read_graph_lifecycle(&storage, &graph.graph_iri).await,
        Some(graph.clone())
    );
    assert_registry_deleted(&storage, group_id, document_id).await;
}
...
#[tokio::test]
async fn delete_requires_tombstone() {
    let (_dir, storage) = test_storage();
...
#[tokio::test]
async fn lifecycle_upsert_idempotent() {
    let (_dir, storage) = test_storage();
    let group_id = Ulid::from_parts(1, 1);
    let document_id = Ulid::from_parts(2, 2);
    ...
    let change =
        aruna_core::storage_entries::lifecycle_revision_change(&lifecycle, node(9), placement);
...
async fn stale_sidecars_block() {
    ...
    assert!(
        store_document_lifecycle(&storage, &local_delete, local_change,)
            .await
            .expect("delete lifecycle applies")
    );
    assert!(
...
// A non-origin publisher (allow_genesis=false) must not mint a missing topic's
// genesis: it gets a retryable error, while the origin creates and publishes.

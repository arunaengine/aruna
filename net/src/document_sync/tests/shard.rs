...
/// reaches the mapping row or the shard manifest: a holder of one shard may not
...
async fn pid_placement_fence() {
    ...
    service
        .ensure_sync_topics(&[valid_topic, forged_topic, other_topic], Vec::new())
...
async fn shard_membership_exact() {
    ...
    service
        .ensure_sync_topics(&[shared_topic], vec![stale_node])
    ...
    assert!(shard_state.members.contains(&node_to_peer(&local_node)));
    assert!(shard_state.members.contains(&node_to_peer(&current_node)));
    assert!(!shard_state.members.contains(&node_to_peer(&stale_node)));
    ...
    assert!(
        shared_state.members.contains(&node_to_peer(&stale_node)),
    ...
    assert!(
        service
            .default_peers
            .read()
            .contains(&node_to_peer(&stale_node)),
...
async fn covered_dependency_retained() {
    ...
    service
        .ensure_sync_topics(&[topic_id], vec![remote_node])
        .expect("shard topic exists");
    ...
    let remote_actor = ::irokle::actor_id_for(topic_id, remote_signer.peer_id());
    oplog
        .create_event_op(
            topic_id,
            remote_actor,
    ...
    let local_op = oplog
        .create_event_op(
            topic_id,
            ::irokle::actor_id_for(topic_id, node_to_peer(&local_node)),
    ...
    let mut cursor = ::irokle::ActorClock::default();
    cursor.observe(
...
async fn stale_publisher_rejected() {
    ...
    let realm_id = restart_realm();
    let receiver = DocumentSyncService::open_with_policy(
    ...
    let publisher = DocumentSyncService::open_with_policy(
    ...
    receiver
        .ensure_sync_topics(&[topic_id], vec![publisher_node])
        .expect("receiver creates shard topic");
    let receiver_ops = ::irokle::oplog::topological(receiver.node().storage(), &topic_id)
        .expect("receiver history reads");
    publisher
        .node()
        .receive_sync_data_from_evicting(
            node_to_peer(&receiver_node),
            SyncData {
                topic_id,
                ops: receiver_ops,
            },
    ...
    let retained_events = ::irokle::oplog::topological(receiver.node().storage(), &topic_id)
...
#[tokio::test]
async fn verification_gates_cutoff() {
    // An unverified shard freezes no former-holder cutoff; a verified shard does:
    // the local clock is a trustworthy cutover boundary only once verified.
    let (_dir, storage) = test_storage();
    let doc = tempfile::tempdir().expect("document sync dir");
    let realm_id = restart_realm();
    let service = DocumentSyncService::open_with_policy(
    ...
    service
        .ensure_sync_topics(&[topic], vec![co_holder])
        .expect("shard topic exists");

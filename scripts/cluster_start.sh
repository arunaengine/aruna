#!/usr/bin/env bash
# Starts the local test cluster from release binaries and waits until the nodes are ready.
...
usage() {
  cat <<'EOF'
Usage: bash scripts/cluster_start.sh [--with-keycloak] [--node-count N] [--portal-dir P]
...
Behavior:
  default          Build the workspace in release mode and launch 3 local Aruna nodes.
                   ...
                   left behind is stopped with scripts/cluster_stop.sh (just stop).

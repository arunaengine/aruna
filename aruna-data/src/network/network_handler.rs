use aruna_net::{
    actor_handle::{NetworkActorHandle, ReceiveStreams}, Kademlia, MaybeSignedAddr
};
...
impl NetworkHandler {
    ...
    pub async fn new(
        ...
        let node_addr = network.get_node_addr().await?;
        let realm = Realm::new(
    ...
    pub async fn create_stream(
        ...
        self.network.create_stream(node_id).await
    }

    #[tracing::instrument(level = "trace", skip(self))]
    pub async fn store(&self, hash: blake3::Hash) -> Result<(), anyhow::Error> {
    ...
    #[tracing::instrument(level = "trace", skip(self))]
    pub async fn find(&self, hash: blake3::Hash) -> Result<Vec<MaybeSignedAddr>, anyhow::Error> {
        self.kademlia.find_value(*hash.as_bytes()).await
    }

    #[tracing::instrument(level = "trace", skip(self))]
    pub async fn get_realm_nodes(&self) -> Result<Vec<NodeAddr>, anyhow::Error> {
        let query = self.realm.get_realm_member_addrs();
        // try one refresh if no members are found
        let members = if query.len() <= 1 {
            self.realm.update_now().await?;

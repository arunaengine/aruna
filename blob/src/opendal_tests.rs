...
async fn azblob_skips_ambient() {
    ...
    let ambient_only =
        build_group_service(GroupBackendKind::Azblob, group_config(&base), &test_guard()).unwrap();
    ...
    let hardened = build_group_service(GroupBackendKind::Azblob, with_key, &test_guard()).unwrap();
    let _ = hardened.stat("probe").await;
...
async fn builds_fs_operator() {
    ...
    let operator =
        build_service::<services::Fs>(HashMap::from([("root".to_string(), root)]), None).unwrap();
...
async fn recursive_list_truncates() {
    ...
    assert_eq!(
        entries[0].kind,
        aruna_core::structs::execution::source_access::SourceEntryKind::File
    );
    assert!(truncated);
}

//! Runs the S3 ListObjectVersions state machine over version rows and served copies.
...
mod test {
    ...
    fn ordered_ulids(count: usize) -> Vec<Ulid> {
        let mut ulids: Vec<Ulid> = (0..count).map(|_| Ulid::generate()).collect();
        ulids.sort();
        ulids
    }
...
mod pure_tests {
    ...
    fn registration() -> Vec<u8> {
        let (_, _, value) = register_entry(crate::blob::managed_copy::CopyRegistration {
            ...
            location: &location(),
            policies: &[policy_ref()],
            origin: aruna_core::structs::storage::blob::CopyOrigin::Write,

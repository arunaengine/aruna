...
async fn apply_metadata_triple(
    ...
    let actor_bytes = postcard::to_allocvec(actor)?;
    let mut doc = automerge::AutoCommit::load(&current)?;
    doc.set_actor((&actor_bytes).into());
    reconcile(&mut doc, &metadata)?;

    write_document(storage, document, doc.save()).await
}

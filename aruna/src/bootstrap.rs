...
fn kind_label(config: &aruna_core::structs::identity::realm::RealmConfigDocument, node_id: NodeId) -> &'static str {
    ...
    match config
        ...
        .map(|node| &node.kind)
    {
        Some(aruna_core::structs::identity::realm::RealmNodeKind::Management) => "management",
        Some(aruna_core::structs::identity::realm::RealmNodeKind::Server) => "server",
        Some(aruna_core::structs::identity::realm::RealmNodeKind::User { .. }) => "user",
        None => "unknown",
    }
}

...
fn has_example(doc: &Value, media: &Value) -> bool {
    ...
    let Some(schema) = media.get("schema") else {
        return values
            .iter()
            .all(|value| !value.is_null() && !value.as_str().is_some_and(|text| text.is_empty()));
    };
...
fn text_gaps(doc: &Value) -> Vec<String> {
    ...
    for (name, operation) in operations(doc) {
        ...
        for (status, response) in responses {
            if !resolve(doc, response).is_some_and(|response| filled(response.get("description"))) {
...
fn description_gaps(name: &str, description: &str, gaps: &mut Vec<String>) {
    ...
    if lead.contains('\n') || !lead.ends_with('.') || length > MAX_LEAD || lead.starts_with("**") {
        gaps.push(format!(
...
async fn accepts_runtime_examples() {
    ...
    let onboarding = serde_json::to_value(aruna_core::onboarding::BootstrapOnboardingResponse {
...
fn rejects_flat_docs() {
    ...
    let lead =
        "Creates a thing and inlines every rule, limit and status in one paragraph. ".repeat(12);

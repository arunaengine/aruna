...
/// Shared state behind the monitoring router.
...
impl MonitoringState {
    /// Builds the monitoring state and registers the built-in scrape-time sources.
    pub async fn new(
        ctx: Arc<DriverContext>,
        metrics: Arc<NodeMetrics>,
        readiness: Readiness,
...
pub fn router(state: Arc<MonitoringState>) -> Router {
    Router::new()
...
/// Serves the monitoring router on an already-bound listener until it stops.
...
async fn healthz(State(state): State<Arc<MonitoringState>>) -> Response {
...
async fn readyz(State(state): State<Arc<MonitoringState>>) -> Response {
...
mod tests {
    ...
    async fn healthz_ok_live() {
        ...
        let ops = MonitoringState::new(ctx, Arc::new(NodeMetrics::new()), Readiness::new()).await;
        let (status, body) = request(&router(ops), "/healthz").await;
    ...
    async fn healthz_fails_dead() {
        ...
        let ops = MonitoringState::new(
            ctx_with_storage(storage),
            Arc::new(NodeMetrics::new()),
            Readiness::new(),
        )
        .await;
        let (status, _body) = request(&router(ops), "/healthz").await;
        assert_eq!(status, StatusCode::SERVICE_UNAVAILABLE);
    }

    #[tokio::test]
    async fn readyz_gates_startup() {
        let (_temp, ctx) = fjall_ctx();
        let readiness = Readiness::new();
        let ops = MonitoringState::new(ctx, Arc::new(NodeMetrics::new()), readiness.clone()).await;
        let router = router(ops);
    ...
    #[tokio::test]
    async fn readyz_storage_failure() {
        let (storage, receivers) = StorageHandle::new();
        drop(receivers);
        let readiness = Readiness::new();
        readiness.set_ready();
        ...
        .await;
        let (status, body) = request(&router(ops), "/readyz").await;
        assert_eq!(status, StatusCode::SERVICE_UNAVAILABLE);
    ...
    async fn readyz_reports_recovery() {
        ...
        let ops = MonitoringState::with_recovery(
            ctx,
            Arc::new(NodeMetrics::new()),
            readiness,
            recovery.clone(),
    ...
    async fn metrics_expose_recovery() {
        ...
        .await;
        let (status, body) = request(&router(ops), "/metrics").await;
        assert_eq!(status, StatusCode::OK);
    ...
    // Invariants 2 and 15: probes decide from local state only, so a context
    // without a net handle still answers liveness without peer/cooldown state.
    #[tokio::test]
    async fn probes_ignore_peers() {
        ...
        let ops =
            MonitoringState::with_recovery(ctx, Arc::new(NodeMetrics::new()), readiness, recovery)
                .await;
        let router = router(ops);
        ...
        // Readiness may fail its own local checks, never a peer, presence or
        // cooldown reason; the recovery object carries an unrelated error class.
        let (_status, body) = request(&router, "/readyz").await;
        let checks = body
            .split("\"checks\":")
            .nth(1)
    ...
    #[tokio::test]
    async fn metrics_expose_storage() {
        let (_temp, ctx) = fjall_ctx();
        let ops = MonitoringState::new(ctx, Arc::new(NodeMetrics::new()), Readiness::new()).await;
        let response = router(ops)
            .oneshot(
                Request::builder()
                    .uri("/metrics")
                    .body(Body::empty())

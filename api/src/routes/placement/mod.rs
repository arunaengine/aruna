...
async fn ensure_placement_read(
    ...
    let realm_reader = crate::auth::permission_granted(
        state,
        auth,
        aruna_core::structs::placement::policy::document::policy_admin_path(auth.realm_id),
    ...
    crate::auth::ensure_permission(
        state,
        auth,
        aruna_core::structs::placement::policy::document::group_admin_path(auth.realm_id, group_id),
        aruna_core::structs::identity::auth::Permission::READ,
    )
    .await
}
...
async fn ensure_placement_writer(
    ...
    let path = match crate::routes::access::groups::get_bucket_group(state, bucket).await? {
        Some(group_id) => {
            aruna_core::structs::placement::policy::document::group_admin_path(realm_id, group_id)
        }
        None => config_admin,
    };
    crate::auth::ensure_permission(
...
async fn local_subject(
    state: &ServerState,
    realm_id: aruna_core::structs::identity::realm::RealmId,
) -> ServerResult<aruna_core::structs::placement::policy::PlacementSubject> {

...
#[utoipa::path(get, path = "/git/{repository}/info/refs", tag = "metadata/git",
    ...
    description = "Requires Aruna bearer authentication or HTTP Basic with that token as password. Authorizes READ for fetch and WRITE for push before advertising refs.",
...
#[utoipa::path(post, path = "/git/{repository}/{service}", tag = "metadata/git",
    ...
    description = "Requires repository READ for upload-pack or WRITE for receive-pack. Git and LFS availability checks run before ref publication. Atomic pushes use Git's native transaction support.",
    params(("repository" = String, Path, description = "Document ID followed by .git"),

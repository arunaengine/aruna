...
#[utoipa::path(
    ...
    request_body = DeletionPreflightRequest,
    responses(
        (status = 200, description = "Bounded, scope-aware deletion inventory and consequences", body = DeletionPreflightResponse),
...
#[utoipa::path(
    ...
    tag = "storage deletion",
    request_body = SubmitPurgeRequest,
    responses(
        (status = 201, description = "Purge job created", body = SubmitPurgeResponse),
        (status = 200, description = "Idempotent replay returned the retained job", body = SubmitPurgeResponse),

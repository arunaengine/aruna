...
**Authentication**: realm bearer token; a token confined to a path subset is refused, because this
surface cannot honor that confinement. READ on the source bucket is checked here, and the target
node checks WRITE on the target bucket before it accepts its half of the relationship.
...
- A prefix, when given, must be a confined relative path."#,
    ...
    responses(
        (
            status = 201,
            description = "The relationship as stored on both nodes; counters are still zero because nothing has replicated yet",
...
**Behavior**
- Only relationships created by the calling user are returned, so this is not a realm-wide view.
- `outgoing` holds relationships whose source bucket lives on this node, `incoming` the mirror
  records this node keeps as a replication target; one relationship therefore appears in `outgoing`
  on one node and in `incoming` on the other.
...
**Limits**
- The read is node-local and unpaginated: every matching record comes back in one response, in no
  guaranteed order."#,
    params(
    ...
    responses(
        (
            status = 200,
            description = "The caller's own relationships held on this node, split by direction; a list is empty when it was filtered out or has no matches",
        ...
        (status = 401, description = "Missing or invalid bearer token", body = ErrorResponse),
        (status = 403, description = "Token from another realm or path-restricted", body = ErrorResponse)
    ),
    security(("bearer_auth" = []))
)]
...
creator may read a relationship, so one created by somebody else is refused rather than hidden.

**Behavior**
- Either end can be inspected on the node that holds it, source-side record or mirror alike.
- `pending_jobs` and `oldest_lag_ms` come from this node's own replication queue: the jobs still
  queued for this relationship, and the age in milliseconds of the oldest of them.
- `pending_jobs` reaching zero with no `last_error` is how a caller sees an accepted run finish; the
  counters advance only as replication reports back, so they lag the objects already written."#,
    ...
    responses(
        (
            status = 200,
            description = "The relationship with this node's queue depth and last outcome; lag, last sync time and last error are omitted when there is nothing to report",
        ...
        (status = 401, description = "Missing or invalid bearer token", body = ErrorResponse),
        (status = 403, description = "Token from another realm or path-restricted, or the relationship was created by another user", body = ErrorResponse),
...
- The 202 means the backfill job is durably queued on this node; nothing has been copied and no
  object has been compared yet.
- The call is idempotent per relationship: the job is keyed by the relationship and its scope, so
  running one while a pass is in flight re-arms the drain over the same job instead of starting a
  second pass.
- `queued` counts the scope jobs enqueued, so it is 1 on every success and is not a count of
  objects.
- Progress is observed by polling the relationship, whose `pending_jobs` returns to zero when the
  run has drained."#,
    params(("id" = String, Path, description = "Relationship id as a 26-character ULID; must name a relationship whose source bucket is on this node")),
    responses(
        (
            status = 202,
            description = "A backfill job over the relationship's whole scope is durably queued; the copying happens afterwards",
...
- The peer is asked to drop its half as part of the call, and a peer that cannot be reached does not
  fail the delete: the removal is retried in the background until the mirror is gone."#,
    ...
    responses(
        (status = 204, description = "The relationship is removed on this node and the peer's mirror is being dropped; synchronized objects are retained on both sides"),
        ...
        (status = 401, description = "Missing or invalid bearer token", body = ErrorResponse),
        (status = 403, description = "Token from another realm or path-restricted, the relationship was created by another user, or no WRITE on this node's endpoint bucket", body = ErrorResponse),

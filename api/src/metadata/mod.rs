...
use aruna_operations::metadata::forward::{
    ...
    route_profile_status as run_profile_validation_status,
};
...
use aruna_operations::metadata::api::{
    ...
    load_realm_nodes, query_select_limit,
};
#[cfg(test)]
...
pub fn router() -> OpenApiRouter<Arc<ServerState>> {
    OpenApiRouter::with_openapi(MetadataApiDoc::openapi())
        .routes(routes!(list_all_documents, create_metadata_document))
        ...
        .routes(routes!(get_validation_status))
        ...
        .routes(routes!(add_data_entity))
        .routes(routes!(add_contextual_entity))
...
fn map_export_view(view: &MetadataRoCrateView) -> OperationMetadataRoCrateExportView {
    match view {
...
mod tests {
  ...
  "@graph": [
    ...
    async fn storage_failure_internal() {
        ...
        let result = load_document_record(state.as_ref(), Ulid::generate()).await;

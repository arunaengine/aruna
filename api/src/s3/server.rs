...
use super::cors::{
    ...
    match_preflight_rule, parse_requested_headers,
};
...
impl Service<Request<Incoming>> for WrappingService {
    ...
    fn call(&self, req: Request<Incoming>) -> Self::Future {
        ...
        let inner = Box::pin(async move {
            // Charge the transport IP before any request validation or body handling.
                ...
                && let Some(origin_header) = origin_header.as_ref()
            {
                ...
                // Bucket rules extend the node allowlist, which remains authoritative for portals.
            ...
            if let Ok(response) = &mut result {
                ...
                if let Some(matched_rule) = bucket_rule {
                    ...
                    // Node-allowlisted origins remain readable when bucket rules omit them.
                    cors.apply_s3_headers(origin_header.as_ref(), response.headers_mut());
                }
            }

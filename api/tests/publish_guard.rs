...
fn allowed_publish_use(publish_use: &PublishUse) -> bool {
    match publish_use.path.as_str() {
        "operations/src/tasks/incoming/outbox.rs" => {
            publish_use.function.as_deref() == Some("publish_from_outbox")
        }
        "net/src/document_sync/publish.rs" => matches!(
        ...
        "net/src/document_sync/eviction.rs" => matches!(
            (publish_use.function.as_deref(), publish_use.text.as_str()),
            (
...
mod fixtures {
    ...
    fn publish_use(path: &str, function: &str, text: &str) -> PublishUse {
        PublishUse {
            path: path.to_owned(),
            line: 1,
    ...
    #[test]
    fn publish_rename_flagged() {
        let renamed = publish_use(
            "operations/src/tasks/incoming/outbox.rs",
            "decode_publish",
            "DocumentSyncPublish::Upsert {",
        );

        assert!(!allowed_publish_use(&renamed));
    }

    #[test]
    fn transport_rename_flagged() {
        let renamed = publish_use(
            "net/src/document_sync/publish.rs",
            "publish_records",
            "DocumentSyncPublish::Upsert {",
        );
    ...
    #[test]
    fn moved_construction_flagged() {
        let moved = publish_use(
            "operations/src/tasks/incoming/child.rs",
            "publish_from_outbox",
            "DocumentSyncPublish::Upsert {",
        );

        assert!(!allowed_publish_use(&moved));
    }

    #[test]
    fn outbox_constructor_allowed() {
        let allowed = publish_use(
            "operations/src/tasks/incoming/outbox.rs",
            "publish_from_outbox",
            "DocumentSyncPublish::Upsert {",
        );

        assert!(allowed_publish_use(&allowed));
    }
}

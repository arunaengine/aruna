...
impl Config {
    fn new() -> Result<Self> {
        Ok(Config {
            socket_addr: dotenvy::var("SOCKET_ADDR")?,
            redirect_url: dotenvy::var("REDIRECT_URL")?,
            issuer_url: dotenvy::var("ISSUER_URL")?,
...
async fn main() {
    ...
    let client = get_oidc_client(config).await.unwrap();
...
async fn entry(
    ...
    if request
        ...
        .is_some()
    {
        todo!("Not sure why I did this? :shrug:")
    } else {
        let (redirect, pkce, csrf, nonce) = oidc_redirect(client.oidc)
            .await
...
async fn callback(
    ...
    //"
    //    );
    let response = Json(json!({"oidc_token": id})).into_response();
    Ok(response)
}

async fn get_oidc_client(config: Config) -> Result<Clients> {
